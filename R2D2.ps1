...
{
    $greet = “Good Morning Michael. Hope you had a cup of coffee already”
}
elseif($Hour –ge 12 –and $Hour –lt 16)
{
    $greet = “Good After noon Michael. How is the weather in Amsterdam?”
}
else
{
    $greet = “Good Evening Michael , I will check the status of this system for you please standby?”
}
...
# This function identifies all services that are 
# configured to auto start with system but are in stopped state
...
{ 
    ...
    $Siri.Speak("$(Get-Greeting)")
    $Siri.Speak("I will check your system Health and give you a status Report")
    $Siri.Speak("$(Get-Uptime)")
    $Siri.Speak("Checking basic hardware performance parameters")
    
    Start-Sleep -Seconds 2
    
    $Siri.Speak("$(Check-HardwarePerformance)")
    $siri.Speak("$(Get-BatteryStatus)")
    $Siri.Speak("$(Get-DiskInfo)")
    $siri.Speak("$(Check-Services)")
    $Siri.Speak("Checking Event logs for errors and warnings. ")
    $Siri.Speak("$(Check-Eventlogs)")
}

# Calling Speak-Siri function to invoke Speach API
Speak-Siri  
